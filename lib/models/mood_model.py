...
def log_mood_to_db(general_feeling, mood, notes):
    import os
    ...
    with sqlite3.connect(db_path) as con:
        sql = "INSERT INTO mood_logs (timestamp, general_feeling, mood, notes) VALUES (DATETIME('now'), ?, ?, ?)"
        cursor = con.cursor()
        cursor.execute(sql, (general_feeling, mood, notes))
        con.commit()
    print("Mood logged!")
